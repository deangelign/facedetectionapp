...
if not os.path.exists(IMAGE_FODLER):
    os.makedirs(IMAGE_FODLER)
...
async def face_detect(filedata: str = File(...)) -> dict:
    ...
    try:
        ...
        return {"face_detected": False, "message": f"Exception raised {e}"}

    return {"face_detected": contain_faces, "message": "Successfully uploaded image"}
...
async def landmark_extraction(filedata: str = File(...)) -> dict:
    ...
    try:
        ...
        _save_image_bytes(filedata, filename)
        img = cv2.imread(filename)
        land_marks_info = face_detector.extract_landmarks(img)
        land_marks: List[Any] = []
        if len(land_marks) > 0:
            land_marks = land_marks_info[0]["landmark_2d_106"]
    except Exception as e:
        return {"land_marks": [], "message": f"Exception raised {e}"}

    return {"land_marks": land_marks, "message": "Successfully uploaded image"}
    ...
    try:
        ...
        if cropped_face is not None:
            obj_base64string = codecs.encode(
                pickle.dumps(cropped_face, protocol=pickle.HIGHEST_PROTOCOL), "base64"
            ).decode("latin1")
        else:
            obj_base64string = None
    except Exception as e:
        return {"cropped_image": [], "message": f"Exception raised {e}"}
    return {
